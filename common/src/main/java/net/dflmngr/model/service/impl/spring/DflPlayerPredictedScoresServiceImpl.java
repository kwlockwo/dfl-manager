...
public class DflPlayerPredictedScoresServiceImpl implements DflPlayerPredictedScoresService {
    ...
    public void insertAll(List<DflPlayerPredictedScores> entities, boolean inTx) {
        // inTx parameter ignored - Spring manages transactions via @Transactional
        repository.saveAll(entities);
    }

    public void updateAll(List<DflPlayerPredictedScores> entities, boolean inTx) {
        // inTx parameter ignored - Spring manages transactions via @Transactional
        repository.saveAll(entities);
    }
    ...
    public Map<Integer, DflPlayerPredictedScores> getForRoundWithKey(int round) {
        List<DflPlayerPredictedScores> predictedScores = repository.findByRound(round);
        Map<Integer, DflPlayerPredictedScores> predictedScoresMap = new HashMap<>();
        for (DflPlayerPredictedScores scores : predictedScores) {
            predictedScoresMap.put(scores.getPlayerId(), scores);
        }
